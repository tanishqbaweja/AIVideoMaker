    ...
    scenes: {
      ...
      items: {
        ...
        properties: {
          ...
          searchQuery: {
            type: "string",
            description: "Concrete Pexels video search query for this visual moment."
          }
        },
...
- For searchQuery, output exactly 1 to 2 words. Focus ONLY on the primary noun or subject. Example: use "wombat" not "wombat walking in nature", use "Eiffel Tower" not "eiffel tower sunny day". Do NOT use subscribe, YouTube, or social-media related queries.
- The script MUST end with "Make sure to Subscribe!" as the very last spoken line.
...
function normalizeScriptPayload(payload: ScriptPayload): ScriptPayload {
  const scenes = payload.scenes.map((scene) => ({
    ...
    searchQuery: scene.searchQuery.trim()
  }));
